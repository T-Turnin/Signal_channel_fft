 ...
 */
  ...
  typedef struct
  {
    uint16_t numTaps;    /**< number of coefficients in the filter. 滤波器中的系数数*/
    float32_t *pState;   /**< points to the state variable array. The array is of length numTaps+blockSize-1. 指向状态变量数组。数组的长度为numTaps+blockSize-1。*/
    float32_t *pCoeffs;  /**< points to the coefficient array. The array is of length numTaps.指向系数数组。该数组的长度为numTaps。 */
    float32_t mu;        /**< step size that controls filter coefficient updates. 控制滤波器系数更新的步长。*/
